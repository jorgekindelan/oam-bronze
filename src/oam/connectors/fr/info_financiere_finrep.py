...
logger = get_logger("oam.fr.info_financiere_finrep")

DEFAULT_API_BASE = (
    "https://www.info-financiere.gouv.fr/api/explore/v2.1"
    "/catalog/datasets/flux-amf-new-prod/records"
)
DEFAULT_WHERE = "sous_type_d_information=\"Rapports financiers et d'audit annuels\""
DEFAULT_PAGE_LIMIT = 100

@register
class FRInfoFinanciereFinRep(BaseConnector):
    country_code = "FR"
    source_code = "INFOFIN_FINREP"
    source_name = "AMF / info-financiere.gouv.fr — Rapports financiers annuels"
    ...
    async def discover(
        ...
        *,
        ...
        where_base = str(extra.get("where_clause") or DEFAULT_WHERE)
        ...
        where = (
            f'{where_base} AND uin_dat_amf >= "{date_from_str}" AND uin_dat_amf <= "{date_to_str}"'
        )
        ...
        async with build_async_client(timeout_s=timeout_s, headers=headers) as client:
            while True:
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"INFOFIN API JSON decode failed: {exc} — preview={preview}"
                    ) from exc
                ...
                for h in hits:
                    # Defensive check: validate filing type
                    if not is_annual_report(h.filing_type_raw):
                        logger.error(
                            "infofin_unexpected_filing_type",
                            extra={"uin_idt_uin": h.uin_idt_uin, "filing_type_raw": h.filing_type_raw}
                        )
                        continue
                    ...
                    if not h.download_url:
                        logger.warning(
                            "infofin_missing_download_url",
                            extra={"uin_idt_uin": h.uin_idt_uin}
                        )
                    ...
                    total_yielded += 1
                    # Store UTC isoformat for consistent ordering (NL pattern).
                    # Using published_at_utc.isoformat() rather than published_at_raw
                    # because raw may carry non-UTC offsets; UTC string comparison is
                    # always monotone for correctly ordered ASC results.
                    ts = h.published_at_utc.isoformat()
                    if watermark is None or ts > watermark:
                        watermark = ts

                logger.info("infofin_page_fetched", extra={
                    "offset": offset, "hits": len(hits), "total_yielded": total_yielded
                })
        ...
        logger.info("infofin_discover_done", extra={"total_yielded": total_yielded})
    ...
    async def download_document(
        ...
        async with build_async_client(timeout_s=timeout_s, headers=headers) as client:
            ...
            # Decompress zip: extract first member (same as NL)
